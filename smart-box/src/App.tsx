...
import { buildDashboardStats } from './analytics/dashboard';
import { DashboardStage } from './dashboard/DashboardStage';
...
type AppMode = 'dispatch' | 'dashboard';
...
function buildVehicleInstruction(task: Task, vehicle: Vehicle, isPrimary: boolean): InstructionItem {
  ...
  const taskLabel = formatTaskLabel(task.id);
  ...
  if (isActiveTask && vehicle.state === 'to-pickup') {
    return {
      ...
      detail: `从 ${current} 去装载 ${taskLabel}，完成后送往 ${dropoff}`,
      meta: `${vehicle.id} · ${taskLabel}`,
      status: '执行中',
      level: 'active',
    };
  }
  ...
  if (isActiveTask && vehicle.state === 'loading') {
    return {
      ...
      detail: `正在装载 ${taskLabel}，装载完成后前往 ${dropoff}`,
      meta: `${vehicle.id} · ${taskLabel}`,
      status: '装货中',
      level: 'active',
    };
  ...
  if (isActiveTask && vehicle.state === 'unloading') {
    return {
      ...
      detail: `正在卸载 ${taskLabel}，完成后本条任务结束`,
      meta: `${vehicle.id} · ${taskLabel}`,
      status: '卸货中',
      level: 'active',
    };
  ...
  if (onboard) {
    return {
      ...
      color: task.color,
      title: `${vehicle.id} 后续送达 ${taskLabel}`,
  ...
  return {
    ...
    color: task.color,
    title: `${vehicle.id} 后续取货 ${taskLabel}`,
...
function buildInstructionItems(snapshot: SimulationSnapshot) {
  ...
  for (const vehicle of snapshot.vehicles) {
    const taskIds = Array.from(
      new Set([...vehicle.routeStops.map((stop) => stop.taskId), ...vehicle.onboardTaskIds]),
    );
...
function PageModeSwitch({ value, onChange }: PageModeSwitchProps) {
  return (
    <div className="page-mode-switch">
      <button
        type="button"
        className={`page-mode-button ${value === 'dispatch' ? 'active' : ''}`}
      ...
      <button
        type="button"
        className={`page-mode-button ${value === 'dashboard' ? 'active' : ''}`}
...
interface DispatchViewProps {
  ...
  appMode: AppMode;
  activeTab: 'tasks' | 'instructions' | 'vehicles';
  onAppModeChange: (mode: AppMode) => void;
  onActiveTabChange: (tab: 'tasks' | 'instructions' | 'vehicles') => void;
      ...
      <section className="panel stage-panel">
        <div className="panel-header">
          <div className="stage-heading">
            <PageModeSwitch value={appMode} onChange={onAppModeChange} />
        ...
        <div className="stage-frame">
          {appMode === 'dispatch' ? (
            ...
            <DashboardStage snapshot={snapshot} stats={stats} />
          )}
        </div>
      </section>
            ...
            <div className="list-block tabbed-block">
              <div className="tab-row">
                ...
                <button
                  ...
                  onClick={() => onActiveTabChange('instructions')}
                >
                  关键指令
                ...
                <button
                  ...
                  onClick={() => onActiveTabChange('vehicles')}
                >
                  车辆状态
                  <span>
                      ...
                      <div key={instruction.id} className="instruction-row">
                        ...
                        <div className="instruction-meta">
                          <div className="instruction-head">
                            ...
                            <span className={`instruction-chip ${instruction.level}`}>{instruction.status}</span>
                          </div>
...
function App() {
  ...
  const dashboardHistory = useDashboardHistory(snapshot);
  const dashboardStats = useMemo(
    () => buildDashboardStats(snapshot, dashboardHistory),
    [snapshot, dashboardHistory],
  );
  ...
  const instructions = useMemo(() => buildInstructionItems(snapshot), [snapshot]);

  return (
    <div className={`app-root ${appMode === 'dashboard' ? 'dashboard-active' : ''}`}>
      <DispatchView
        snapshot={snapshot}
        ...
        showVehicleRoutes={showVehicleRoutes}
        onToggleRoutes={() => setShowVehicleRoutes((current) => !current)}
        stageMode={stageMode}
